;( function ( $ ) {
	...
	// Typography control
	wp.customize.controlConstructor[ 'shopwell-typography' ] = wp.customize.Control.extend(
		{
			...
			ready: function () {
				...
				control.container.find('.shopwell-font-color-control').wpColorPicker({
				...
				control.container.find( '.reset-defaults' ).on(
					...
					function () {
						...
						rangeFields.forEach(
							( item ) => {
                            control.container.find( '[data-option-id="' + item + '"]' ).find( '.shopwell-reset-range' ).click();
							}
						);
					}
				);
			...
			update_font_weight_field: function ( font_family ) {
				...
				if ( 'inherit' === font_family || 'default' === font_family ) {
					...
					$.each(
						shopwell_typography_vars.fonts,
				...
				self.container.find( '.shopwell-typography-font-weight' ).find( 'select' ).html( options );
			},
			...
			update_font_subsets_field: function ( font_family ) {
				...
				selected = '',
				setting  = self.setting.get(),
				field    = self.container.find( '.shopwell-typography-font-subsets' );
