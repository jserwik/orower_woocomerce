...
if ( ! class_exists( 'Shopwell_Customizer_Control_Sortable' ) ) :
	/**
	 * Shopwell Customizer custom select control class.
	 */
	class Shopwell_Customizer_Control_Sortable extends Shopwell_Customizer_Control {
		...
		protected function content_template() {
			?>
				<div class="shopwell-control-wrapper shopwell-sortable-wrapper">
					...
					<ul class="shopwell-sortable-control">
						<# for ( key in data.value ) { #>
							<li class="shopwell-sortable-item<# if ( _.has( data.value, key) && ! data.value[ key ] || ! _.has( data.value, key ) && _.has( data.default, key ) && ! data.default[ key ] ) { #> invisible<# } #>" data-value="{{ key }}">
								<# if ( data.sortable ) { #>
