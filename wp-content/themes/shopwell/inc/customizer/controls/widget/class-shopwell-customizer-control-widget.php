<?php
/**
 * Shopwell Customizer Widget control class.
 *
 * @package     Shopwell
...
if ( ! class_exists( 'Shopwell_Customizer_Control_Widget' ) ) :
	/**
	 * Shopwell Customizer custom select control class.
	 */
	class Shopwell_Customizer_Control_Widget extends Shopwell_Customizer_Control {
		...
		public function to_json() {
			...
			if ( is_array( $this->widgets ) && ! empty( $this->widgets ) ) {
				...
				// Get all widgets.
				$widgets = shopwell_get_customizer_widgets();
				...
				foreach ( $this->widgets as $widget_id => $args ) {
					...
					ob_start();
					?>
					<div data-widget-id="<?php echo esc_attr( $instance->id_base ); ?>" class="shopwell-widget-tpl-<?php echo esc_attr( $instance->id_base ); ?> shopwell-widget" data-max-uses=<?php echo esc_attr( $max_uses ); ?>>
		...
		protected function content_template() {
			?>
			<div class="shopwell-control-wrapper shopwell-widget-wrapper">

				<div class="shopwell-widget-container">

					<# _.each( data.value, function( template, i ) { #>
						{{{ template }}}
					<# }); #>

				</div><!-- END .shopwell-widget-container -->

				<div class="shopwell-add-widget-wrap">			
					<button type="button" class="button shopwell-add-widget" data-location-title="{{{ data.label }}}" data-control="{{{ data.id }}}"><?php esc_html_e( 'Add Widget', 'shopwell' ); ?></button>
				</div>

				<div class="shopwell-widget-tmpls">
