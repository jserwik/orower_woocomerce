...
function ShopwellLinkField($el) {
	...
	const open = () => {
		...
		var $textarea = jQuery( '<textarea id="shopwell-link-textarea" style="display:none;"></textarea>' );
		if ( ! jQuery( '#shopwell-link-textarea' ).length) {
			jQuery( 'body' ).append( $textarea );
		}

		var val = getNodeValue();
		wpLink.open( 'shopwell-link-textarea', val.url, val.title, null );
	}
	...
	const  onClose = () => {
		...
		jQuery( '#shopwell-link-textarea' ).remove();
	}
...
( function ( api, $ ) {
	api.controlConstructor['shopwell-repeater'] = api.Control.extend(
		{
			ready: function () {
				var control = this;
				setTimeout(
			...
			_init: function () {
				...
				control.autocorrect_range_input_number = function ( input_number, timeout ) {
					...
					reset           = parseFloat( range.find( '.shopwell-reset-range' ).attr( 'data-reset_value' ) ),
				...
				if ( ! window._upload_shopwell ) {
					...
					window._upload_shopwell = wp.media(
						{
				...
				window._upload_shopwell.on(
					'close',
					function () {

						// get selections and save to hidden input plus other AJAX stuff etc.
						var selection = window._upload_shopwell.state().get( 'selection' );
				...
				window._upload_shopwell.on(
					...
					function () {
						...
						var media_attachment = window._upload_shopwell.state().get( 'selection' ).first().toJSON();
				...
				control.handleGradient = function ( $context ) {
					...
					range_input,
					value,
					shopwell_range_input_number_timeout,
					...
					$( '.item-gradient', $context ).each(
						function () {
							...
							_item.find( '.shopwell-range-wrapper' ).each(
								function () {
									var $this = $( this );
									...
									$this.find( 'input.shopwell-range-input' ).on(
										'change keyup',
										function () {
											control.autocorrect_range_input_number( $( this ), 1000, shopwell_range_input_number_timeout );
										}
									).on(
										'focusout',
										function () {
											control.autocorrect_range_input_number( $( this ), 0, shopwell_range_input_number_timeout );
										}
									);
				...
				control.linkPicker = function ( $context ) {
					if (item_link = $context.find( '.shopwell-field-link' )) {
						const a = new ShopwellLinkField( item_link );
						a.init();
					}
				}
				...
				control.actions = function ( $context ) {
					if ( control.params.live_title_id ) {
						...
						if ( 'shopwell_section_order_styling' === control.id ) {
...
jQuery( document ).ready(
	function ( $ ) {
		...
		icon_picker.find( '.c-icon-search input' ).attr( 'placeholder', Shopwell_Icon_Picker.search );
