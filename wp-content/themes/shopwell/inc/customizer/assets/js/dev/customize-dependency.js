;( function( $ ) {
	...
	var shopwellDependencies = {
		...
		init: function() {
			...
			wp.customize.control.each( function( control ) {
				self.showShopwellControl( control );
			});
					...
					wp.customize( master, function( setting ) {
						var setupControl = function( control ) {
							...
							isDisplayed = function() {
								return self.showShopwellControl( wp.customize.control( slave ) );
							};
		 ...
		 * @returns {bool}
		 */
		showShopwellControl: function( control ) {
			var self     = this,
				show     = true,
				i;
		 ...
		 *
		 * @since 1.0.0
		 * @param {Object} requirement - The requirement, inherited from showShopwellControl.
	...
	}; // END var shopwellDependencies

	$( document ).ready( function() {
		shopwellDependencies.init();
	});
