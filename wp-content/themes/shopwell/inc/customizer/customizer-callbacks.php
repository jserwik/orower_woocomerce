 ...
 */
function shopwell_sanitize_number( $val ) {
 ...
 */
function shopwell_sanitize_toggle( $input, $setting ) {

	if ( 'shopwell_enable_front_page' === $setting ) {
		sanitize_key( $input );
	}
 ...
 * @return boolean
 */
function shopwell_sanitize_checkbox( $input ) {
...
function shopwell_sanitize_radio( $input, $setting ) {
	// Ensure input is a slug
	$input = sanitize_key( $input );
 ...
 * @param object $setting  Setting object.
 */
function shopwell_sanitize_textarea( $input, $setting ) {
 ...
 */
function shopwell_sanitize_color( $color ) {
	...
	if ( false === strpos( $color, 'rgba' ) ) {
		return shopwell_sanitize_hex_color( $color );
	}

	return shopwell_sanitize_alpha_color( $color );
}
 ...
 */
function shopwell_sanitize_hex_color( $color ) {
 ...
 */
function shopwell_sanitize_alpha_color( $color ) {
 ...
 * @param object $setting  Setting object.
 */
function shopwell_sanitize_image( $image, $setting ) {
	/*
 ...
 */
function shopwell_sanitize_widget( $widgets, $setting ) {
 ...
 */
function shopwell_no_sanitize( $val ) {
	return $val;
}
 ...
 * @return string
 */
function shopwell_sanitize_order( $input ) {
	...
	$filter_empty = array_filter( $json, 'shopwell_not_empty' );
 ...
 * @return bool
 */
function shopwell_not_empty( $val ) {
	return ! empty( $val );
}
 ...
 */
function shopwell_sanitize_background( $background ) {
 ...
 */
function shopwell_sanitize_spacing( $values, $setting, $default = array() ) {
 ...
 * @param mixed  $default Default Value.
 */
function shopwell_sanitize_range( $value, $setting, $default = '' ) {
 ...
 * @param object $setting WP Customize Setting instance.
 */
function shopwell_sanitize_responsive( $values, $setting ) {

	$control       = $setting->manager->get_control( $setting->id );
	$control_type  = str_replace( 'shopwell-', '', $control->type );
	...
	if ( is_array( $control->responsive ) && ! empty( $control->responsive ) ) {
		...
		foreach ( $values as $device => $value ) {
			...
			$values[ $device ] = call_user_func_array(
				'shopwell_sanitize_' . $control_type,
				array(
					$values[ $device ],
					$setting,
 ...
 * @param object $setting  Setting object.
 */
function shopwell_sanitize_typography( $value, $setting ) {
 ...
 * @param object $setting  Setting object.
 */
function shopwell_sanitize_design_options( $value, $setting ) {
	...
	if ( isset( $control->display ) ) {
		...
		if ( isset( $control->display['background'] ) ) {
			...
			if ( array_key_exists( 'color', $control->display['background'] ) ) {
				...
				if ( isset( $value['background-color'] ) ) {
					$sanitized['background-color'] = shopwell_sanitize_color( $value['background-color'] );
				}
			}
			...
			if ( array_key_exists( 'gradient', $control->display['background'] ) ) {
				...
				if ( isset( $value['gradient-color-1'] ) ) {
					$sanitized['gradient-color-1'] = shopwell_sanitize_color( $value['gradient-color-1'] );
				}

				if ( isset( $value['gradient-color-2'] ) ) {
					$sanitized['gradient-color-2'] = shopwell_sanitize_color( $value['gradient-color-2'] );
				}

				if ( isset( $value['gradient-color-1-location'] ) ) {
					$sanitized['gradient-color-1-location'] = shopwell_sanitize_range( $value['gradient-color-1-location'], $setting );
				}

				if ( isset( $value['gradient-color-2-location'] ) ) {
					$sanitized['gradient-color-2-location'] = shopwell_sanitize_range( $value['gradient-color-2-location'], $setting );
				}
				...
				if ( isset( $value['gradient-linear-angle'] ) ) {
					$sanitized['gradient-linear-angle'] = shopwell_sanitize_range( $value['gradient-linear-angle'], $setting );
				}
		...
		if ( isset( $control->display['color'] ) ) {
			foreach ( $control->display['color'] as $id => $title ) {
				if ( isset( $value[ $id ] ) ) {
					$sanitized[ $id ] = shopwell_sanitize_color( $value[ $id ] );
				}
			}
		}
		...
		if ( isset( $control->display['border'] ) ) {
			...
			if ( array_key_exists( 'color', $control->display['border'] ) && isset( $value['border-color'] ) ) {
				$sanitized['border-color'] = shopwell_sanitize_color( $value['border-color'] );
			}
			...
			if ( isset( $value['separator-color'] ) ) {
				$sanitized['separator-color'] = shopwell_sanitize_color( $value['separator-color'] );
			}
 ...
 * @param object $setting  Setting object.
 */
function shopwell_sanitize_sortable( $value, $setting ) {
 ...
 */
function shopwell_repeater_sanitize( $input, $setting ) {
	$control = $setting->manager->get_control( $setting->id );
			...
			if ( isset( $fields[ $id ] ) ) {
				switch ( strtolower( $fields[ $id ]['type'] ) ) {
					...
					case 'editor':
						...
						break;
					case 'color':
						$data[ $i ][ $id ] = shopwell_sanitize_hex_color( $value );
						break;
					case 'coloralpha':
						$data[ $i ][ $id ] = shopwell_sanitize_alpha_color( $value );
						break;
					case 'checkbox':
						$data[ $i ][ $id ] = shopwell_sanitize_checkbox( $value );
						break;
					case 'select':
						$data[ $i ][ $id ] = '';
					...
					case 'gradient':
						...
						break;
					case 'design-options':
						shopwell_sanitize_design_options( $value, $setting );
						break;
					default:
						$data[ $i ][ $id ] = wp_kses_post( $value );
				}
