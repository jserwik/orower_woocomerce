...
<div id="dokan-store-listing-filter-wrap">
	...
	<div class="left">
		<div class="item">
			<button class="dokan-store-list-filter-button dokan-btn dokan-btn-theme">
				<?php echo \Shopwell\Icon::get_svg( 'filter' ); ?>
				<?php esc_html_e( 'Filter', 'shopwell' ); ?>
			</button>
		</div>
		...
		<p class="item store-count">
			...
			printf(
				esc_html(
					_n(
						'Total store showing: %s',
						'Total stores showing: %s',
						$number_of_store,
						'shopwell'
					)
				),
				number_format_i18n($number_of_store)
			);
	...
	<div class="right">
		...
		<form name="stores_sorting" class="sort-by item" method="get">
			<label><?php esc_html_e( 'Sort by', 'shopwell' ); ?>:</label>

			<select name="stores_orderby" id="stores_orderby" aria-label="<?php esc_attr_e( 'Sort by', 'shopwell' ); ?>">
		...
		<div class="toggle-view item">
			<?php echo \Shopwell\Icon::get_svg( 'view-medium', 'ui', array( 'data-view' => 'grid-view' ) ); ?>
			<?php echo \Shopwell\Icon::get_svg( 'view-list', 'ui', array( 'data-view' => 'list-view' ) ); ?>
		</div>
...
<form role="store-list-filter" method="get" name="dokan_store_lists_filter_form" id="dokan-store-listing-filter-form-wrap" style="display: none">
	...
	if ( apply_filters( 'dokan_load_store_lists_filter_search_bar', true ) ) :
		...
		<div class="store-search grid-item">
			<input type="search" class="store-search-input" name="dokan_seller_search" placeholder="<?php esc_attr_e( 'Search Vendors', 'shopwell' ); ?>">
		</div>
		<?php
	endif;
	...
	<div class="apply-filter">
		<button id="cancel-filter-btn" class="dokan-btn dokan-btn-theme"><?php esc_html_e( 'Cancel', 'shopwell' ); ?></button>
		<button id="apply-filter-btn" class="dokan-btn dokan-btn-theme" type="submit"><?php esc_html_e( 'Apply', 'shopwell' ); ?></button>
	</div>
